        ...
        foreach($myimage->MandateImage as $theimage ){
            if ($countimage<$maximages){
                ...
                if (!is_null($url) && $url<> "" ){
                    ...
                    $certificate=ABSPATH .$pluginpath."/propctrl-com.pem";
                   ...
                   $arrContextOptions=array(
                    ...
                    if ($countimage<$imagestodisplay){
                        ...
                        $cl=$cl+1;                           
                        $html.= "<img  class='column' src='$thm_disp' >";
                        if ($cl==6){
                            $cl=0;
                            $html.="</div>";
                        }
...
function watermark($src, $dst, $wm){
    
    //Load the watermark PNG using imagecreatefrompng
$watermark = imagecreatefromgif($wm);

//Load our image using imagecreatefromjpeg
...
//Figure out the $dst_x value.
$dstX = ($photoWidth - $watermarkWidth - $marginSide);

//Figure out the $dst_y value.
$dstY = ($photoHeight - $watermarkHeight - $marginBottom);


//Copy our watermark PNG onto the image using the 
//coordinates and size dimensions we calculated above.
imagecopy($photo, $watermark, $dstX, $dstY, 0, 0, $watermarkWidth, $watermarkHeight);

//Output the new image to the browser.
//header('Content-type: image/jpeg');
//imagejpeg($photo);
ImageJPEG($photo, $dst, 95); 
//Finally, destroy the image resource and free up memory.
imagedestroy($photo);
...
function Thumbnail($url, $filename, $width = 3000, $height = true){
...
function Thumbnails($url, $filename, $width = 150, $height = true) {

 // download and create gd image
 $image = ImageCreateFromString(file_get_contents_curl($url));
...
function loggedon()
{
    global $current_user;
    wp_get_current_user();
    if ($current_user->user_login<>""){return $current_user->user_login; }else{return "";}
